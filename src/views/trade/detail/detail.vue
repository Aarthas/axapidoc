    ...
    <div>
        ...
        <div style="display:flex;flex-direction:column;background-color: #ffffff;margin-top: 8px;">
            ...
            <div style="display: flex;flex-direction: row;padding-bottom: 10px;">
                <x-button v-if="showCancel" @click.native="cancelOrder(itemsData.orderId)"
                          style=" margin-left:8px;width:100px;height: 35px;line-height: 32px;font-size: 13px;margin-top: 10px;margin-right: 8px;">
                    取消订单
                </x-button>
                <x-button v-if="itemsData.orderStatus==5" @click.native="goPay"
                          style="margin-left:8px;width:100px;height: 35px;line-height: 32px;font-size: 13px;margin-top: 10px;margin-right: 8px;"
                          type="primary">去支付
                </x-button>
                <x-button v-if="showlogistics" @click.native="checkLogistics(itemsData.orderId)"
                          style=" margin-left:8px;width:100px;height: 35px;line-height: 32px;font-size: 13px;margin-top: 10px;margin-right: 8px;">
                    查看物流
                </x-button>
...
//    let orderId = Lib.Utils.getQueryString("orderId");
    ...
    export default {
        ...
        computed: {
            showCancel: function () {
                if (page.itemsData) {
                    console.log("aaa"+page.itemsData)
                    if (page.itemsData.orderStatus == 5)

                        return true;
                    else if (page.itemsData.orderStatus > 5 && page.itemsData.orderStatus < 25) {
                        return true;
                    }
                }
                return false;
            ...
            showlogistics: function () {
                if (page.itemsData.orderStatus) {
                    if (page.itemsData.orderStatus == 60)
                        return true;
                    else if (page.itemsData.orderStatus > 5 && page.itemsData.orderStatus <= 50) {
                        return true;
                    }
                }
                return false;
